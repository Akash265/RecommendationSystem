...
def load_edge_csv(df,
    ...
    # get user_ids from rating events in the order of occurrance
    src = [user_id for user_id in df[src_index_col]]
    # get movie_id from rating events in the order of occurrance
...
edge_index = load_edge_csv(rating_df, 'userId', 'movieId', 'rating',rating_threshold=3.5)
...
# --- Convert between R-matrix and adjacency-matrix formats ---
...
def sample_mini_batch(batch_size, edge_index):
    """Random samples indices of a minibatch given an adjacency matrix
    ...
    """
    # Random samples indices of a minibatch of positive and negative samples chosen with replacement.

    # Return a k sized list of population elements chosen with replacement.
    # if the population is empty, raises IndexError.

    batch_size = min(batch_size, edge_index.shape[1])

    user_indices, pos_item_indices, neg_item_indices = structured_negative_sampling(
        edge_index, contains_neg_self_loops=False)

    # 3 x edge_index len
    edges = torch.stack([user_indices, pos_item_indices, neg_item_indices], dim=0)

    # here is when we actually perform the batch sampe
    # Return a k sized list of population elements chosen with replacement.
    # if the population is empty, raises IndexError.
    # note that random.choices returns a list of indices, hence we need to sample the first batch_size samples from the edge
...
class LightGCN(MessagePassing):
    ...
    def __init__(self, num_users,
        ...
        # embedding dimension: num_users/num_items x embedding_dim
    ...
    def forward(self, edge_index: Tensor):
        ...
        this is essentially a matrix operation way to get 1/ (sqrt(n_neighbors_i) * sqrt(n_neighbors_j))
    ...
    def message(self, x_j, norm):
      ...
      #
      # x_j is basically the embedding of all the neighbors based on the src_list in coo edge_index
      #
...
def get_metrics(model,
    ...
    for exclude_edge_index in exclude_edge_indices:
        ...
        for user, items in user_pos_items.items():
            # (user) * len(items) can give us [user1, user1, user1...] with len of len(items)
    ...
    # convert test_user_pos_items dictionary into a list of lists
    test_user_pos_items_list = [test_user_pos_items[user.item()] for user in users]

       # here is "pred_relevant_items n actually_relevant_items" list for each user
    r = []
    for user in users:
        user_true_relevant_item = test_user_pos_items[user.item()]
        # list of Booleans to store whether or not a given item in the top k items for a given user
        # is also present in user_true_relevant_item.
        # this is later on used to compute n_rel and rec_k
...
def evaluation(model,
    ...
    Args:
        ...
        exclude_edge_indices (list): 2 by N list of edges for split to discount from evaluation
        k (int): determines the top k items to compute metrics on
        lambda_val (float): determines lambda value for bpr loss
